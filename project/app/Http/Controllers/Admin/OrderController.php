        ...
        elseif($status == 'declined') {
         ...
         return Datatables::of($datas)
                            ...
                            ->editColumn('pay_amount', function(Order $data) {
                                return $data->currency_sign . round($data->pay_amount * $data->currency_value);
                            })
                            ->editColumn('created_at', function(Order $data) {
                                return Carbon::parse($data->created_at)->format('d-M-Y h:i:s a');
                            })
                            ->addColumn('order_status', function(Order $data) {
                                if ($data->status=='pending' || $data->status=='processing'){
                                    return '<span class="badge badge-info">'.$data->status.'</span>';
                                }elseif($data->status=='on delivery' || $data->status=='completed'){
...
<a href="javascript:;" class="send" data-email="'. $data->customer_email .'" data-toggle="modal" data-target="#vendorform"><i class="fas fa-envelope"></i> Send</a>
<a href="javascript:;" data-href="' . route('admin-order-delete', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> Delete</a>
<a href="javascript:;" data-href="'. route('admin-order-track',$data->id) .'" class="track" data-toggle="modal" data-target="#modal1"><i class="fas fa-truck"></i> Track Order</a>'.$orders.'</div></div>';
                            })
                            ->rawColumns(['id','action','order_status'])
    ...
    public function destroy($id){
        ...
        if ($data->vendororders->count() > 0) {
            foreach ($data->vendororders as $vord) {
                $vord->delete();
            }
        }
        if ($data->tracks->count() > 0) {
            foreach ($data->tracks as $otrack) {
                $otrack->delete();
            }
        }
        $data->delete();

...
class GeneralSettingController extends Controller
{
    ...
    public function cod($status)
    {
     ...
     public function evb($status)
    {

        $data = Generalsetting::findOrFail(1);
        $data->evb_check = $status;
        $data->update();
    }
    ...
    public function isrefferal($status)
    {
        $data = Generalsetting::findOrFail(1);
        $data->is_refferal = $status;
        $data->update();
    }

    public function isorderrefferal($status)
    {
        $data = Generalsetting::findOrFail(1);
        $data->is_order_referral = $status;
        $data->update();
    }



    public function is_min_order($status)
    {
        $data = Generalsetting::findOrFail(1);
        $data->is_min_order = $status;
        $data->update();
    }

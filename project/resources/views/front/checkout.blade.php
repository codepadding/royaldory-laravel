            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                    ...
                    <form id="" action="" method="POST" class="checkoutform">
                                                ...
                                                <div class="row">
                                                    <div class="col-lg-12">
                                                        <div class="nav flex-column" role="tablist"
                                                            ...
                                                            @if($gs->evb_check == 1 && auth()->user())
                                                                @if(auth()->user()->balance >= $gs->evb_limit)
                                                                    @if(round($totalPrice * $curr->value) <= auth()->user()->balance)
                                                                        <a class="nav-link payment" data-val=""
                                                                           data-show="no"
                                                                           data-form="{{route('cash.submit')}}"
                                                                           data-href="{{ route('front.load.payment',['slug1' => 'evb','slug2' => 0]) }}"
                                                                           ...
                                                                           aria-selected="false">
                                                                            ...
                                                                            <p>Royaldory Balance
                                                                                <small>Pay with evendory
                                                                                    balance @if(auth()->user()) Up to
                                                                                    Tk {{auth()->user()->balance}} @endif</small>
                                                                            </p>
                                                                        </a>
                        ...
                        <input type="hidden" id="dp" name="dp" value="{{$digital}}">
                        <input type="hidden" id="tax" name="tax" value="{{$gs->tax}}">
                        <input type="hidden" id="totalQty" name="totalQty" value="{{$totalQty}}">

                        <input type="hidden" name="vendor_shipping_id" value="{{ $vendor_shipping_id }}" id="vendor_shipping_id">
                        <input type="hidden" name="vendor_packing_id" value="{{ $vendor_packing_id }}" id="vendor_packing_id">
                                        ...
                                        <form id="loginform" action="{{ route('user.login.submit') }}" method="POST">
                                            ...
                                            <div class="form-input">
                                                <input type="password" class="Password" name="password"
                                                       placeholder="{{ $langg->lang174 }}" value="1234" required="">
...
$('#comment-log-reg1').modal('show');
        ...
        $('.shipping').on('click', function () {
            ...
            var user_balance="{{auth()->user()?auth()->user()->balance : 0}}";
            if(user_balance < ttotal){
                $('#v-pills-tab11-tab').hide().removeClass('active');
                $('#v-pills-tab3-tab').addClass('active');
                $('#v-pills-tab3-tab').trigger('click');
            }else{
                $('#v-pills-tab11-tab').show();
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data == 0) {
                        ...
                        //if promo applied then hide evb and select cashon delivery and others
                        $('#v-pills-tab11-tab').hide().removeClass('active');
                        $('#v-pills-tab3-tab').addClass('active');
                        $('#v-pills-tab3-tab').trigger('click');
...
<script defer>
    ...
    obj.shipping = $('#shipop').val();
    obj.pickup_location = $('#pickup_location').val();
    obj.name = $('#name').val();
    obj.phone = $('#phone').val();
    obj.email = $('#email').val();
    ...
    obj.city = $('#city').val();
    obj.zip = $('#zip').val();
    obj.total = $('#grandtotal').val();
    obj.shipping_name = $('#shippingFull_name').val();
    obj.shipping_address = $('#shipping_address').val();
    obj.shipping_phone = $('#shipingPhone_number').val();
    obj.shipping_email = $('#shipping_email').val();

...
<div class="invoice-wrap">
    ...
    <div class="invoice__metaInfo" style="justify-content: center; margin-top: 0">
        <div class="col-lg-6">
            <div class="invoice__orderDetails">
                <table>
                    <tr>
                        <td class="text-right"><strong>{{ __('Invoice Number') }} :</strong></td>
                        <td>{{ sprintf("%'.08d", $order->id) }}</strong></td>
                    </tr>
                    <tr>
                        <td class="text-right"><strong>{{ __('Order Date') }} :</strong></td>
                        <td>{{ date('d-M-Y',strtotime($order->created_at)) }}</strong></td>
                    </tr>
                    <tr>
                        <td class="text-right">{{  __('Order ID')}} :</td>
                        <td>{{ $order->order_number }}</td>
                    </tr>
                    @if($order->dp == 0)
                        <tr>
                            <td class="text-right">{{ __('Shipping Method') }} :</td>
                            <td>
                    ...
                    @endif
                    <tr>
                        <td class="text-right">{{ __('Payment Method') }} :</td>
                        <td>{{$order->method}}</td>
                    </tr>
                </table>
            </div>
                ...
                <div class="table-responsive">
                    <table id="example2" class="table table-hover dt-responsive" cellspacing="0"
                        ...
                        @if($order->shipping_cost != 0)
                            @php
                                $unroundedShippingPrice=$order->shipping_cost / $order->currency_value;
                                $price = round(($order->shipping_cost / $order->currency_value),2);
                            @endphp
                            @if(DB::table('shippings')->where('price','=',$unroundedShippingPrice)->count() > 0)
                                <tr class="no-border">
                                    <td colspan="1"></td>
                                    <td><strong>{{ DB::table('shippings')->where('price','=',$unroundedShippingPrice)->first()->title }}
                        ...
                        @if($order->packing_cost != 0)
                            @php
                                $unroundedPprice=$order->packing_cost / $order->currency_value;
                                $pprice = round(($order->packing_cost / $order->currency_value),2);
                            @endphp
                            @if(DB::table('packages')->where('price','=',$unroundedPprice)->count() > 0)
                                <tr class="no-border">
                                    <td colspan="1"></td>
                                    <td><strong>{{ DB::table('packages')->where('price','=',$unroundedPprice)->first()->title }}
                                            ({{$order->currency_sign}})</strong></td>
                        ...
                        <tr class="final-border">
                            ...
                            <td>{{$order->currency_sign}}{{ round($order->pay_amount * $order->currency_value) }}
                            </td>
                        </tr>
